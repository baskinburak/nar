    ...
    namespace MessageTypes {
        namespace WaitChunkPull {
            ...
            class Request : public RequestHeader {
                ...
                public:
                    Request(): RequestHeader(std::string("wait_chunk_pull_request")) {}
                    Request(unsigned short rport,long long int sid, long long int cid, long long int csize): RequestHeader(std::string("wait_chunk_pull_request")), rand_port(rport), stream_id(sid), chunk_id(cid), chunk_size(csize){}
                    long long int get_stream_id();
                    unsigned short get_randevous_port();
                    long long int  get_chunk_id();
                    long long int  get_chunk_size();
                    void send_mess(nar::Socket* skt,nar::MessageTypes::WaitChunkPull::Response & resp);
