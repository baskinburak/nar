...
std::string& nar::Messagetypes::WaitChunkPull::Request::get_stream_id() {
    return stream_id;
}
...
void nar::Messagetypes::WaitChunkPull::Request::send_mess(nar::Socket* skt) {
    ...
    receive_message(wpull_req_recv);
    return;
}
...
void nar::Messagetypes::WaitChunkPull::Response::send_mess(nar::Socket* skt) {
    ...
    std::string temp = get_message(skt);
    nlohmann::json wpull_resp_recv = nlohmann::json::parse(temp);
    receive_message(wpull_resp_recv);
    return;
}
