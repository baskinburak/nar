...
CREATE TABLE IF NOT EXISTS `nar`.`TimeTable` (
  `machine_id` VARCHAR(150) NOT NULL,
  `i0_1` FLOAT NULL DEFAULT 0,
  `i1_2` FLOAT NULL DEFAULT 0,
  `i2_3` FLOAT NULL DEFAULT 0,
  `i3_4` FLOAT NULL DEFAULT 0,
  `i4_5` FLOAT NULL DEFAULT 0,
  `i5_6` FLOAT NULL DEFAULT 0,
  `i6_7` FLOAT NULL DEFAULT 0,
  `i7_8` FLOAT NULL DEFAULT 0,
  `i8_9` FLOAT NULL DEFAULT 0,
  `i9_10` FLOAT NULL DEFAULT 0,
  `i10_11` FLOAT NULL DEFAULT 0,
  `i11_12` FLOAT NULL DEFAULT 0,
  `i12_13` FLOAT NULL DEFAULT 0,
  `i13_14` FLOAT NULL DEFAULT 0,
  `i14_15` FLOAT NULL DEFAULT 0,
  `i15_16` FLOAT NULL DEFAULT 0,
  `i16_17` FLOAT NULL DEFAULT 0,
  `i17_18` FLOAT NULL DEFAULT 0,
  `i18_19` FLOAT NULL DEFAULT 0,
  `i19_20` FLOAT NULL DEFAULT 0,
  `i20_21` FLOAT NULL DEFAULT 0,
  `i21_22` FLOAT NULL DEFAULT 0,
  `i22_23` FLOAT NULL DEFAULT 0,
  `i23_24` FLOAT NULL DEFAULT 0,
...
DELIMITER $$
USE `nar`$$
CREATE DEFINER = CURRENT_USER TRIGGER `nar`.`Sessions_AFTER_UPDATE` AFTER UPDATE ON `Sessions` FOR EACH ROW
BEGIN
	SET @latest = (Select last_update From TimeTable Where NEW.machine_id=TimeTable.machine_id);
	SET @pre_count = (Select session_count From TimeTable Where NEW.machine_id=TimeTable.machine_id);
	SET @ses_count = @pre_count;
	SET @is_ses = DATEDIFF(@NEW.join_time,@latest);
    SET @to_add = DATEDIFF(NEW.leave_time,NEW.join_time);
    IF @is_ses <> 0 THEN
		SET @ses_count = @ses_count + @to_add;
        SET @ses_count = @ses_count + 1;
	END IF;
